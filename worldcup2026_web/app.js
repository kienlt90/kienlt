...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function attachMatchCardEvents() {
    ...
    // 4. Click event on match card to open starting lineups (livescore style)
    document.querySelectorAll(".match-card").forEach(card => {
      card.addEventListener("click", (e) => {
        // Prevent opening if user clicks inputs, buttons, or controls inside the card
        if (e.target.closest("input") || e.target.closest("button") || e.target.closest(".score-input-container")) {
          return;
        }
        const matchId = card.id.replace("match-card-", "");
        openLineupsModal(matchId);
      });
  ...
  const NAME_POOL = {
    spanish: {
      first: ["Carlos", "Jose", "Juan", "Luis", "Francisco", "Antonio", "Manuel", "Pedro", "Jesus", "Angel", "Diego", "Mateo", "Santiago", "Sebastian", "Gabriel", "Emiliano", "Alejandro"],
      last: ["Rodriguez", "Gonzalez", "Gomez", "Fernandez", "Lopez", "Diaz", "Martinez", "Perez", "Garcia", "Sanchez", "Romero", "Alvarez", "Torres", "Ruiz", "Ramirez", "Flores", "Acosta"]
    },
    portuguese: {
      first: ["Lucas", "Joao", "Pedro", "Matheus", "Gabriel", "Thiago", "Bruno", "Felipe", "Rafael", "Rodrigo", "Andre", "Gustavo", "Guilherme", "Leonardo", "Arthur", "Bernardo"],
      last: ["Silva", "Santos", "Oliveira", "Souza", "Rodrigues", "Ferreira", "Almeida", "Costa", "Gomes", "Martins", "Araujo", "Ribeiro", "Carvalho", "Cardoso", "Teixeira", "Pinto"]
    },
    english: {
      first: ["John", "James", "Robert", "Michael", "William", "David", "Richard", "Joseph", "Thomas", "Charles", "Marcus", "Bukayo", "Mason", "Harry", "Jack", "Jordan", "Declan"],
      last: ["Smith", "Johnson", "Williams", "Brown", "Jones", "Miller", "Davis", "Wilson", "Anderson", "Taylor", "Thomas", "Moore", "Jackson", "Martin", "White", "Harris", "Clark"]
    },
    german_dutch: {
      first: ["Thomas", "Andreas", "Michael", "Stefan", "Christian", "Martin", "Frank", "Lars", "Sven", "Jan", "Luuk", "Frenkie", "Virgil", "Memphis", "Joshua", "Leon", "Florian"],
      last: ["Müller", "Schmidt", "Schneider", "Fischer", "Weber", "Meyer", "Wagner", "Becker", "Schulz", "Hoffmann", "de Jong", "van Dijk", "de Ligt", "Dumfries", "Kimmich"]
    },
    french: {
      first: ["Jean", "Pierre", "Michel", "Philippe", "Kylian", "Antoine", "Olivier", "Hugo", "Lucas", "Adrien", "Aurélien", "Theo", "Benjamin", "Dayot", "William", "Kolo"],
      last: ["Martin", "Bernard", "Dubois", "Thomas", "Robert", "Richard", "Petit", "Durand", "Leroy", "Moreau", "Mbappé", "Griezmann", "Giroud", "Lloris", "Hernandez", "Saliba"]
    },
    nordic_slavic: {
      first: ["Patrik", "Tomáš", "Erling", "Martin", "Luka", "Ivan", "Andrej", "Mateo", "Edin", "Miralem", "Dejan", "Alexander", "Viktor", "Emil", "Kristoffer", "Jonas"],
      last: ["Souček", "Schick", "Haaland", "Ødegaard", "Modrić", "Kramarić", "Kovačić", "Perišić", "Džeko", "Pjanić", "Isak", "Gyökeres", "Kulusevski", "Lindelöf", "Olsen"]
    },
    arabic: {
      first: ["Mohamed", "Ahmed", "Ali", "Hassan", "Youssef", "Salem", "Akram", "Almoez", "Aymen", "Sadio", "Nicolas", "Firas", "Yazan", "Hakim", "Achraf"],
      last: ["Salah", "El-Neny", "Al-Dawsari", "Afif", "Ali", "Hussein", "Jasim", "Mané", "Jackson", "Ziyech", "Hakimi", "En-Nesyri", "Aboubakar", "Mahrez", "Bounedjah"]
    },
    asian: {
      first: ["Heung-min", "Hee-chan", "Kang-in", "Kaoru", "Takefusa", "Ayase", "Mehdi", "Sardar", "Eldor", "Otabek", "Min-jae", "Wataru", "Junya", "Koit", "Shokhboz"],
      last: ["Son", "Hwang", "Lee", "Mitoma", "Kubo", "Ueda", "Taremi", "Azmoun", "Shomurodov", "Shukurov", "Kim", "Endo", "Ito", "Alireza", "Masharipov"]
    },
    others: {
      first: ["Juninho", "Rangelo", "Cuco", "Gervane", "Kenji", "Vurnon", "Jafar", "Ramsley", "Bradley", "Leandro"],
      last: ["Bacuna", "Janga", "Martina", "Kastaneer", "Gorré", "Anita", "Arias", "Bonifacia", "Kuwas", "Gaari"]
    }
  };

  function getLanguageGroup(teamName) {
    const mapping = {
      // Spanish
      "Mexico": "spanish", "Paraguay": "spanish", "Tây Ban Nha": "spanish", "Colombia": "spanish", 
      "Uruguay": "spanish", "Ecuador": "spanish", "Argentina": "spanish", "Panama": "spanish",
      // Portuguese
      "Brazil": "portuguese", "Bồ Đào Nha": "portuguese", "Cape Verde": "portuguese",
      // English
      "Mỹ": "english", "Canada": "english", "Scotland": "english", "Úc": "english", 
      "Anh": "english", "New Zealand": "english", "Ghana": "english", "Nam Phi": "english",
      // German / Dutch
      "Đức": "german_dutch", "Hà Lan": "german_dutch", "Thụy Sĩ": "german_dutch", "Áo": "german_dutch",
      // French
      "Pháp": "french", "Bỉ": "french", "Senegal": "french", "Haiti": "french", 
      "Bờ Biển Ngà": "french", "CH Dân Chủ Congo": "french",
      // Nordic / Slavic
      "Na Uy": "nordic_slavic", "Thụy Điển": "nordic_slavic", "Cộng hòa Séc": "nordic_slavic", 
      "Croatia": "nordic_slavic", "Bosnia-Herzegovina": "nordic_slavic",
      // Arabic
      "Saudi Arabia": "arabic", "Qatar": "arabic", "Morocco": "arabic", "Algeria": "arabic", 
      "Tunisia": "arabic", "Ai Cập": "arabic", "Jordan": "arabic", "Iraq": "arabic",
      // Asian
      "Hàn Quốc": "asian", "Nhật Bản": "asian", "Iran": "asian", "Uzbekistan": "asian"
    };
    return mapping[teamName] || "others";
  }
  ...
  function generateLineupForTeam(teamName, matchId) {
    const group = getLanguageGroup(teamName);
    const pool = NAME_POOL[group] || NAME_POOL.others;
    const topPlayers = TEAM_PLAYERS[teamName] || [];
    const seedBase = getSeed(matchId + teamName);
    
    // Determine formation
    const formations = ["4-3-3", "4-2-3-1", "3-5-2"];
    const formIdx = Math.floor(seededRandom(seedBase) * formations.length);
    const formation = formations[formIdx];
    
    let positions = [];
    if (formation === "4-3-3") {
      positions = [
        { pos: "GK", label: "Thủ môn" },
        { pos: "LB", label: "Hậu vệ trái" },
        { pos: "CB", label: "Trung vệ" },
        { pos: "CB", label: "Trung vệ" },
        { pos: "RB", label: "Hậu vệ phải" },
        { pos: "CM", label: "Tiền vệ" },
        { pos: "CM", label: "Tiền vệ" },
        { pos: "AM", label: "Tiền vệ công" },
        { pos: "LW", label: "Tiền đạo cánh" },
        { pos: "RW", label: "Tiền đạo cánh" },
        { pos: "ST", label: "Tiền đạo trung lộ" }
      ];
      ...
      positions = [
        ...
        { pos: "CB", label: "Trung vệ" },
        { pos: "RB", label: "Hậu vệ phải" },
        { pos: "DM", label: "Tiền vệ phòng ngự" },
        { pos: "DM", label: "Tiền vệ phòng ngự" },
        { pos: "AM", label: "Tiền vệ công" },
        { pos: "LW", label: "Tiền đạo cánh" },
        { pos: "RW", label: "Tiền đạo cánh" },
      ...
      positions = [
        ...
        { pos: "CB", label: "Trung vệ" },
        { pos: "CB", label: "Trung vệ" },
        { pos: "LM", label: "Tiền vệ trái" },
        { pos: "CM", label: "Tiền vệ trung tâm" },
        { pos: "DM", label: "Tiền vệ phòng ngự" },
        { pos: "CM", label: "Tiền vệ trung tâm" },
        { pos: "RM", label: "Tiền vệ phải" },
        { pos: "ST", label: "Tiền đạo" },
        { pos: "ST", label: "Tiền đạo" }
      ];
    }
    ...
    const topPlayersPlaced = [];
    
    function getRandomName(idx) {
      const fIdx = Math.floor(seededRandom(seedBase + idx * 17) * pool.first.length);
      const lIdx = Math.floor(seededRandom(seedBase + idx * 31) * pool.last.length);
      if (group === "asian") {
        return `${pool.last[lIdx]} ${pool.first[fIdx]}`;
      }
      return `${pool.first[fIdx]} ${pool.last[lIdx]}`;
    }
    
    positions.forEach((p, index) => {
      if (topPlayers.length > 0 && (p.pos === "ST" || p.pos === "LW" || p.pos === "RW" || p.pos === "AM" || p.pos === "CM") && topPlayersPlaced.length < topPlayers.length) {
        const topP = topPlayers[topPlayersPlaced.length];
        players.push({
          number: index === 0 ? 1 : (topPlayersPlaced.length === 0 ? 10 : (topPlayersPlaced.length === 1 ? 7 : 9)),
          name: topP.name,
          pos: p.pos,
          label: p.label,
          isStar: true
        });
        usedNames.add(topP.name);
        topPlayersPlaced.push(topP);
      } else {
        let name = getRandomName(index);
        let attempts = 0;
        while (usedNames.has(name) && attempts < 10) {
          name = getRandomName(index + attempts * 100);
          attempts++;
        }
        usedNames.add(name);
        
        let number = index + 1;
        if (p.pos === "GK") number = 1;
        else if (p.pos === "LB") number = 3;
        else if (p.pos === "RB") number = 2;
        else if (p.pos === "CB" && index === 2) number = 4;
        else if (p.pos === "CB" && index === 3) number = 5;
        else if (p.pos === "DM") number = 6;
        else if (p.pos === "CM" && index === 5) number = 8;
        else if (p.pos === "CM" && index === 6) number = 14;
        else if (p.pos === "AM") number = 10;
        else if (p.pos === "LW") number = 11;
        else if (p.pos === "RW") number = 7;
        else if (p.pos === "ST") number = 9;
        
        while (players.some(pl => pl.number === number)) {
          number = Math.floor(seededRandom(seedBase + number * 43) * 22) + 12;
        }
    ...
    const benchPositions = [
      { pos: "GK", label: "Thủ môn dự bị" },
      { pos: "DF", label: "Hậu vệ dự bị" },
      { pos: "MF", label: "Tiền vệ dự bị" },
      { pos: "FW", label: "Tiền đạo dự bị" },
      { pos: "FW", label: "Tiền đạo dự bị" }
    ];

    benchPositions.forEach((bp, index) => {
      let name = getRandomName(index + 50);
      let attempts = 0;
      while (usedNames.has(name) && attempts < 10) {
        name = getRandomName(index + 50 + attempts * 100);
        attempts++;
      }
      usedNames.add(name);

      let number = Math.floor(seededRandom(seedBase + index * 57 + 200) * 15) + 12;
      while (players.some(pl => pl.number === number) || bench.some(b => b.number === number)) {
        number = (number + 1) % 30 || 12;
      }
    ...
    const coachFirst = pool.first[Math.floor(seededRandom(seedBase + 500) * pool.first.length)];
    const coachLast = pool.last[Math.floor(seededRandom(seedBase + 501) * pool.last.length)];
    let coachName = group === "asian" ? `${coachLast} ${coachFirst}` : `${coachFirst} ${coachLast}`;
  ...
  function renderLineupLists(l1, l2) {
    const col1 = document.getElementById("lineup-col-team1");
    const col2 = document.getElementById("lineup-col-team2");
    if (!col1 || !col2) return;
    
    // Team 1
    let html1 = `
      <div class="team-header-lineup">
        <span class="team-name-flag">
          <span>Đội hình xuất phát</span>
        </span>
        <span class="formation-badge">${l1.formation}</span>
      </div>
      <div class="player-list-section">
    `;
    l1.startingXI.forEach(p => {
      html1 += `
        <div class="player-row ${p.isStar ? 'star' : ''}">
    ...
    html1 += `
      ...
      <div class="player-list-section" style="margin-top: 15px;">
        <div class="player-list-title">Danh sách dự bị</div>
    `;
    l1.bench.forEach(p => {
      html1 += `
        <div class="player-row" style="opacity: 0.8; padding: 6px 12px;">
          <div class="player-info-left">
            <div class="player-number" style="background: rgba(255, 255, 255, 0.02);">${p.number}</div>
            <span class="player-name-text" style="font-size: 13px;">${p.name}</span>
          </div>
          <span class="player-pos-badge" style="font-size: 9px;">${p.pos}</span>
        </div>
      `;
    });
    html1 += `
      ...
      <div class="coach-info">
        <span style="color: var(--text-muted);">HLV Trưởng:</span>
        <strong>${l1.coach}</strong>
      </div>
    `;
    ...
    let html2 = `
      <div class="team-header-lineup">
        ...
        <span class="formation-badge">${l2.formation}</span>
      </div>
      <div class="player-list-section">
    `;
    l2.startingXI.forEach(p => {
      html2 += `
        <div class="player-row ${p.isStar ? 'star' : ''}">
          <div class="player-info-left">
            <div class="player-number">${p.number}</div>
    ...
    html2 += `
      ...
      <div class="coach-info">
        <span style="color: var(--text-muted);">HLV Trưởng:</span>
        <strong>${l2.coach}</strong>
      </div>
    `;
    col2.innerHTML = html2;
  }

  function renderLineupPitch(l1, l2) {
    ...
    const coords1_433 = {
      "GK": [90, 50],
      "LB": [74, 15], "CB1": [78, 38], "CB2": [78, 62], "RB": [74, 85],
      "CM1": [62, 30], "CM2": [62, 70], "AM": [54, 50],
      "LW": [40, 20], "RW": [40, 80], "ST": [33, 50]
    };
    const coords1_4231 = {
      "GK": [90, 50],
      "LB": [74, 15], "CB1": [78, 38], "CB2": [78, 62], "RB": [74, 85],
      "DM1": [65, 33], "DM2": [65, 67], "AM": [53, 50],
      "LW": [40, 20], "RW": [40, 80], "ST": [33, 50]
    };
    const coords1_352 = {
      "GK": [90, 50],
      "CB1": [78, 25], "CB2": [80, 50], "CB3": [78, 75],
      "LM": [62, 15], "CM1": [58, 33], "DM": [68, 50], "CM2": [58, 67], "RM": [62, 85],
      "ST1": [33, 38], "ST2": [33, 62]
    };

    const coords2_433 = {
      "GK": [10, 50],
      "LB": [26, 85], "CB2": [22, 62], "CB1": [22, 38], "RB": [26, 15],
      "CM2": [38, 70], "CM1": [38, 30], "AM": [46, 50],
      "LW": [60, 80], "RW": [60, 20], "ST": [67, 50]
    };
    const coords2_4231 = {
      "GK": [10, 50],
      "LB": [26, 85], "CB2": [22, 62], "CB1": [22, 38], "RB": [26, 15],
      "DM2": [35, 67], "DM1": [35, 33], "AM": [47, 50],
      "LW": [60, 80], "RW": [60, 20], "ST": [67, 50]
    };
    const coords2_352 = {
      "GK": [10, 50],
      "CB3": [22, 75], "CB2": [20, 50], "CB1": [22, 25],
      "RM": [38, 85], "CM2": [42, 67], "DM": [32, 50], "CM1": [42, 33], "LM": [38, 15],
      "ST2": [67, 62], "ST1": [67, 38]
    };

    const getPosList = (form) => {
      if (form === "4-3-3") return ["GK", "LB", "CB1", "CB2", "RB", "CM1", "CM2", "AM", "LW", "RW", "ST"];
      if (form === "4-2-3-1") return ["GK", "LB", "CB1", "CB2", "RB", "DM1", "DM2", "AM", "LW", "RW", "ST"];
      return ["GK", "CB1", "CB2", "CB3", "LM", "CM1", "DM", "CM2", "RM", "ST1", "ST2"];
    };

    // Render Team 1
    const posList1 = getPosList(l1.formation);
    const coordsMap1 = l1.formation === "4-3-3" ? coords1_433 : (l1.formation === "4-2-3-1" ? coords1_4231 : coords1_352);
    
    l1.startingXI.forEach((p, idx) => {
      const posCode = posList1[idx];
      const coord = coordsMap1[posCode] || [80, 50];
      
      const div = document.createElement("div");
      div.className = `pitch-player team-1 ${p.isStar ? 'star' : ''}`;
      div.style.top = `${coord[0]}%`;
      div.style.left = `${coord[1]}%`;
      div.title = `${p.name} (${p.pos})`;
      
      const lastName = p.name.split(" ").slice(-1)[0];
      div.innerHTML = `
        <div class="player-shirt">${p.number}</div>
        <div class="player-shirt-label">${lastName}</div>
      `;
      container.appendChild(div);
    });

    // Render Team 2
    const posList2 = getPosList(l2.formation);
    const coordsMap2 = l2.formation === "4-3-3" ? coords2_433 : (l2.formation === "4-2-3-1" ? coords2_4231 : coords2_352);
    
    l2.startingXI.forEach((p, idx) => {
      const posCode = posList2[idx];
      const coord = coordsMap2[posCode] || [20, 50];
      
      const div = document.createElement("div");
      div.className = `pitch-player team-2 ${p.isStar ? 'star' : ''}`;
      div.style.top = `${coord[0]}%`;
      div.style.left = `${coord[1]}%`;
      div.title = `${p.name} (${p.pos})`;
  ...
  function openLineupsModal(matchId) {
    const match = matches.find(m => m.id === matchId);
    if (!match) return;
    
    const lineup1 = generateLineupForTeam(match.team1, match.id);
    const lineup2 = generateLineupForTeam(match.team2, match.id);
    
    const titleEl = document.getElementById("lineup-modal-title");
    if (titleEl) {
      titleEl.innerHTML = `
        <div style="display: flex; align-items: center; justify-content: center; gap: 15px; font-size: 15px; flex-wrap: wrap;">
          <div style="display: flex; align-items: center; gap: 8px;">
            <img src="https://flagcdn.com/w40/${match.team1FlagCode}.png" style="width: 28px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.1);" />
            <span style="font-weight: 800; color: #fff; font-size: 16px;">${match.team1}</span>
          </div>
          <div style="font-size: 22px; font-weight: 900; color: var(--primary); background: rgba(0,0,0,0.2); padding: 4px 14px; border-radius: 8px;">
            ${match.score1 !== null ? match.score1 : '-'} : ${match.score2 !== null ? match.score2 : '-'}
          </div>
          <div style="display: flex; align-items: center; gap: 8px;">
            <span style="font-weight: 800; color: #fff; font-size: 16px;">${match.team2}</span>
            <img src="https://flagcdn.com/w40/${match.team2FlagCode}.png" style="width: 28px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.1);" />
          </div>
        </div>
        <div style="font-size: 11px; color: var(--text-muted); text-align: center; margin-top: 6px; font-weight: 600; letter-spacing: 0.5px;">
          BẢNG ${match.group} • VÒNG ${match.round} • TRẬN ${match.id} • ${match.date} ${match.time.replace(':', 'h')}
        </div>
      `;
    }
    
    renderLineupLists(lineup1, lineup2);
    renderLineupPitch(lineup1, lineup2);
    
    const firstTabBtn = document.querySelector(".lineup-tab-btn[data-tab='list']");
