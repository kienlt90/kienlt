// Dữ liệu CHÍNH THỨC 48 đội bóng và 12 bảng đấu World Cup 2026 (Có bổ sung flagCode phục vụ hiển thị ảnh lá cờ)
const WORLD_CUP_DATA = {
  groups: {
    A: [
      { id: "MEX", name: "Mexico", code: "MEX", flagCode: "mx", flag: "🇲🇽" },
      { id: "KOR", name: "Hàn Quốc", code: "KOR", flagCode: "kr", flag: "🇰🇷" },
      { id: "RSA", name: "Nam Phi", code: "RSA", flagCode: "za", flag: "🇿🇦" },
      { id: "CZE", name: "Cộng hòa Séc", code: "CZE", flagCode: "cz", flag: "🇨🇿" }
    ],
    B: [
      { id: "CAN", name: "Canada", code: "CAN", flagCode: "ca", flag: "🇨🇦" },
      { id: "SUI", name: "Thụy Sĩ", code: "SUI", flagCode: "ch", flag: "🇨🇭" },
      { id: "QAT", name: "Qatar", code: "QAT", flagCode: "qa", flag: "🇶🇦" },
      { id: "BIH", name: "Bosnia-Herzegovina", code: "BIH", flagCode: "ba", flag: "🇧🇦" }
    ],
    C: [
      { id: "BRA", name: "Brazil", code: "BRA", flagCode: "br", flag: "🇧🇷" },
      { id: "MAR", name: "Morocco", code: "MAR", flagCode: "ma", flag: "🇲🇦" },
      { id: "SCO", name: "Scotland", code: "SCO", flagCode: "gb-sct", flag: "🏴" },
      { id: "HAI", name: "Haiti", code: "HAI", flagCode: "ht", flag: "🇭🇹" }
    ],
    D: [
      { id: "USA", name: "Mỹ", code: "USA", flagCode: "us", flag: "🇺🇸" },
      { id: "PAR", name: "Paraguay", code: "PAR", flagCode: "py", flag: "🇵🇾" },
      { id: "AUS", name: "Úc", code: "AUS", flagCode: "au", flag: "🇦🇺" },
      { id: "TUR", name: "Thổ Nhĩ Kỳ", code: "TUR", flagCode: "tr", flag: "🇹🇷" }
    ],
    E: [
      { id: "GER", name: "Đức", code: "GER", flagCode: "de", flag: "🇩🇪" },
      { id: "ECU", name: "Ecuador", code: "ECU", flagCode: "ec", flag: "🇪🇨" },
      { id: "CIV", name: "Bờ Biển Ngà", code: "CIV", flagCode: "ci", flag: "🇨🇮" },
      { id: "CUW", name: "Curacao", code: "CUW", flagCode: "cw", flag: "🇨🇼" }
    ],
    F: [
      { id: "NED", name: "Hà Lan", code: "NED", flagCode: "nl", flag: "🇳🇱" },
      { id: "JPN", name: "Nhật Bản", code: "JPN", flagCode: "jp", flag: "🇯🇵" },
      { id: "TUN", name: "Tunisia", code: "TUN", flagCode: "tn", flag: "🇹🇳" },
      { id: "SWE", name: "Thụy Điển", code: "SWE", flagCode: "se", flag: "🇸🇪" }
    ],
    G: [
      { id: "BEL", name: "Bỉ", code: "BEL", flagCode: "be", flag: "🇧🇪" },
      { id: "IRN", name: "Iran", code: "IRN", flagCode: "ir", flag: "🇮🇷" },
      { id: "EGY", name: "Ai Cập", code: "EGY", flagCode: "eg", flag: "🇪🇬" },
      { id: "NZL", name: "New Zealand", code: "NZL", flagCode: "nz", flag: "🇳🇿" }
    ],
    H: [
      { id: "ESP", name: "Tây Ban Nha", code: "ESP", flagCode: "es", flag: "🇪🇸" },
      { id: "URU", name: "Uruguay", code: "URU", flagCode: "uy", flag: "🇺🇾" },
      { id: "KSA", name: "Saudi Arabia", code: "KSA", flagCode: "sa", flag: "🇸🇦" },
      { id: "CPV", name: "Cape Verde", code: "CPV", flagCode: "cv", flag: "🇨🇻" }
    ],
    I: [
      { id: "FRA", name: "Pháp", code: "FRA", flagCode: "fr", flag: "🇫🇷" },
      { id: "SEN", name: "Senegal", code: "SEN", flagCode: "sn", flag: "🇸🇳" },
      { id: "NOR", name: "Na Uy", code: "NOR", flagCode: "no", flag: "🇳🇴" },
      { id: "IRQ", name: "Iraq", code: "IRQ", flagCode: "iq", flag: "🇮🇶" }
    ],
    J: [
      { id: "ARG", name: "Argentina", code: "ARG", flagCode: "ar", flag: "🇦🇷" },
      { id: "AUT", name: "Áo", code: "AUT", flagCode: "at", flag: "🇦🇹" },
      { id: "ALG", name: "Algeria", code: "ALG", flagCode: "dz", flag: "🇩🇿" },
      { id: "JOR", name: "Jordan", code: "JOR", flagCode: "jo", flag: "🇯🇴" }
    ],
    K: [
      { id: "POR", name: "Bồ Đào Nha", code: "POR", flagCode: "pt", flag: "🇵🇹" },
      { id: "COL", name: "Colombia", code: "COL", flagCode: "co", flag: "🇨🇴" },
      { id: "UZB", name: "Uzbekistan", code: "UZB", flagCode: "uz", flag: "🇺🇿" },
      { id: "COD", name: "CH Dân Chủ Congo", code: "COD", flagCode: "cd", flag: "🇨🇩" }
    ],
    L: [
      { id: "ENG", name: "Anh", code: "ENG", flagCode: "gb-eng", flag: "🏴" },
      { id: "CRO", name: "Croatia", code: "CRO", flagCode: "hr", flag: "🇭🇷" },
      { id: "PAN", name: "Panama", code: "PAN", flagCode: "pa", flag: "🇵🇦" },
      { id: "GHA", name: "Ghana", code: "GHA", flagCode: "gh", flag: "🇬🇭" }
    ]
  }
};


// Dữ liệu lịch thi đấu chính thức vòng bảng World Cup 2026 từ ESPN API (72 trận đấu)
const OFFICIAL_MATCHES_RAW = [
  {
    id: "M01",
    group: "A",
    round: 1,
    date: "12/06/2026",
    timestamp: 1781204400000,
    time: "02:00",
    team1Id: "MEX",
    team2Id: "RSA",
    score1: 2,
    score2: 0,
    yc1: 1,
    rc1: 1,
    yc2: 2,
    rc2: 2,
    status: "Kết thúc",
    scorers1: [{ name: "Julián Quiñones", min: "9'" }, { name: "Raúl Jiménez", min: "67'" }],
    scorers2: [],
    assists1: ["Érik Lira", "Roberto Alvarado"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M02",
    group: "A",
    round: 1,
    date: "12/06/2026",
    timestamp: 1781229600000,
    time: "09:00",
    team1Id: "KOR",
    team2Id: "CZE",
    score1: 2,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Hwang In-Beom", min: "67'" }, { name: "Oh Hyeon-Gyu", min: "80'" }],
    scorers2: [{ name: "Ladislav Krejcí", min: "59'" }],
    assists1: ["Lee Kang-In", "Hwang In-Beom"],
    assists2: ["Vladimír Coufal"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M03",
    group: "B",
    round: 1,
    date: "13/06/2026",
    timestamp: 1781290800000,
    time: "02:00",
    team1Id: "CAN",
    team2Id: "BIH",
    score1: 1,
    score2: 1,
    yc1: 2,
    rc1: 0,
    yc2: 3,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Cyle Larin", min: "78'" }],
    scorers2: [{ name: "Jovo Lukic", min: "21'" }],
    assists1: ["Promise David"],
    assists2: ["Sead Kolasinac"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M04",
    group: "D",
    round: 1,
    date: "13/06/2026",
    timestamp: 1781312400000,
    time: "08:00",
    team1Id: "USA",
    team2Id: "PAR",
    score1: 4,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 5,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Damián Bobadilla (OG)", min: "7'" }, { name: "Folarin Balogun", min: "31'" }, { name: "Folarin Balogun", min: "45'+5'" }, { name: "Giovanni Reyna", min: "90'+8'" }],
    scorers2: [{ name: "Maurício", min: "73'" }],
    assists1: ["Christian Pulisic", "Malik Tillman", "Alex Freeman"],
    assists2: ["Julio Enciso"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M05",
    group: "B",
    round: 1,
    date: "14/06/2026",
    timestamp: 1781377200000,
    time: "02:00",
    team1Id: "QAT",
    team2Id: "SUI",
    score1: 1,
    score2: 1,
    yc1: 2,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Miro Muheim (OG)", min: "90'+4'" }],
    scorers2: [{ name: "Breel Embolo", min: "17'" }],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M06",
    group: "C",
    round: 1,
    date: "14/06/2026",
    timestamp: 1781388000000,
    time: "05:00",
    team1Id: "BRA",
    team2Id: "MAR",
    score1: 1,
    score2: 1,
    yc1: 2,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Vinícius Júnior", min: "32'" }],
    scorers2: [{ name: "Ismael Saibari", min: "21'" }],
    assists1: ["Bruno Guimarães"],
    assists2: ["Brahim Díaz"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M07",
    group: "C",
    round: 1,
    date: "14/06/2026",
    timestamp: 1781398800000,
    time: "08:00",
    team1Id: "HAI",
    team2Id: "SCO",
    score1: 0,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 3,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "John McGinn", min: "28'" }],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M08",
    group: "D",
    round: 1,
    date: "14/06/2026",
    timestamp: 1781409600000,
    time: "11:00",
    team1Id: "AUS",
    team2Id: "TUR",
    score1: 2,
    score2: 0,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Nestory Irankunda", min: "27'" }, { name: "Connor Metcalfe", min: "75'" }],
    scorers2: [],
    assists1: ["Paul Okon-Engstler"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M09",
    group: "E",
    round: 1,
    date: "15/06/2026",
    timestamp: 1781456400000,
    time: "00:00",
    team1Id: "GER",
    team2Id: "CUW",
    score1: 7,
    score2: 1,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Felix Nmecha", min: "6'" }, { name: "Nico Schlotterbeck", min: "38'" }, { name: "Kai Havertz", min: "45'+5'" }, { name: "Jamal Musiala", min: "47'" }, { name: "Nathaniel Brown", min: "68'" }, { name: "Deniz Undav", min: "78'" }, { name: "Kai Havertz", min: "88'" }],
    scorers2: [{ name: "Livano Comenencia", min: "21'" }],
    assists1: ["Florian Wirtz", "Nathaniel Brown", "Joshua Kimmich", "Deniz Undav", "Joshua Kimmich", "Deniz Undav"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M10",
    group: "F",
    round: 1,
    date: "15/06/2026",
    timestamp: 1781467200000,
    time: "03:00",
    team1Id: "NED",
    team2Id: "JPN",
    score1: 2,
    score2: 2,
    yc1: 3,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Virgil van Dijk", min: "51'" }, { name: "Crysencio Summerville", min: "64'" }],
    scorers2: [{ name: "Keito Nakamura", min: "57'" }, { name: "Daichi Kamada", min: "89'" }],
    assists1: ["Ryan Gravenberch", "Ryan Gravenberch"],
    assists2: ["Takefusa Kubo", "Koki Ogawa"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M11",
    group: "E",
    round: 1,
    date: "15/06/2026",
    timestamp: 1781478000000,
    time: "06:00",
    team1Id: "CIV",
    team2Id: "ECU",
    score1: 1,
    score2: 0,
    yc1: 3,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Amad Diallo", min: "90'" }],
    scorers2: [],
    assists1: ["Wilfried Singo"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M12",
    group: "F",
    round: 1,
    date: "15/06/2026",
    timestamp: 1781488800000,
    time: "09:00",
    team1Id: "SWE",
    team2Id: "TUN",
    score1: 5,
    score2: 1,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Yasin Ayari", min: "7'" }, { name: "Alexander Isak", min: "30'" }, { name: "Viktor Gyökeres", min: "59'" }, { name: "Mattias Svanberg", min: "84'" }, { name: "Yasin Ayari", min: "90'+6'" }],
    scorers2: [{ name: "Omar Rekik", min: "43'" }],
    assists1: ["Viktor Gyökeres", "Alexander Isak", "Alexander Isak", "Lucas Bergvall"],
    assists2: ["Hannibal Mejbri"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M13",
    group: "H",
    round: 1,
    date: "15/06/2026",
    timestamp: 1781539200000,
    time: "23:00",
    team1Id: "ESP",
    team2Id: "CPV",
    score1: 0,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M14",
    group: "G",
    round: 1,
    date: "16/06/2026",
    timestamp: 1781550000000,
    time: "02:00",
    team1Id: "BEL",
    team2Id: "EGY",
    score1: 1,
    score2: 1,
    yc1: 2,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Mohamed Hany (OG)", min: "66'" }],
    scorers2: [{ name: "Emam Ashour", min: "20'" }],
    assists1: [],
    assists2: ["Mohamed Salah"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M15",
    group: "H",
    round: 1,
    date: "16/06/2026",
    timestamp: 1781560800000,
    time: "05:00",
    team1Id: "KSA",
    team2Id: "URU",
    score1: 1,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Abdulelah Al-Amri", min: "41'" }],
    scorers2: [{ name: "Maxi Araújo", min: "80'" }],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M16",
    group: "G",
    round: 1,
    date: "16/06/2026",
    timestamp: 1781571600000,
    time: "08:00",
    team1Id: "IRN",
    team2Id: "NZL",
    score1: 2,
    score2: 2,
    yc1: 1,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Ramin Rezaeian", min: "32'" }, { name: "Mohammad Mohebbi", min: "64'" }],
    scorers2: [{ name: "Elijah Just", min: "7'" }, { name: "Elijah Just", min: "54'" }],
    assists1: ["Ramin Rezaeian"],
    assists2: ["Chris Wood", "Chris Wood"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M17",
    group: "I",
    round: 1,
    date: "17/06/2026",
    timestamp: 1781636400000,
    time: "02:00",
    team1Id: "FRA",
    team2Id: "SEN",
    score1: 3,
    score2: 1,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Kylian Mbappé", min: "66'" }, { name: "Bradley Barcola", min: "82'" }, { name: "Kylian Mbappé", min: "90'+6'" }],
    scorers2: [{ name: "Ibrahim Mbaye", min: "90'+5'" }],
    assists1: ["Michael Olise", "Adrien Rabiot"],
    assists2: ["Iliman Ndiaye"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M18",
    group: "I",
    round: 1,
    date: "17/06/2026",
    timestamp: 1781647200000,
    time: "05:00",
    team1Id: "IRQ",
    team2Id: "NOR",
    score1: 1,
    score2: 4,
    yc1: 1,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Aymen Hussein", min: "39'" }],
    scorers2: [{ name: "Erling Haaland", min: "29'" }, { name: "Erling Haaland", min: "43'" }, { name: "Leo Østigard", min: "76'" }, { name: "Aymen Hussein (OG)", min: "90'+6'" }],
    assists1: ["Amir Al-Ammari"],
    assists2: ["David Møller Wolfe", "Martin Ødegaard"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M19",
    group: "J",
    round: 1,
    date: "17/06/2026",
    timestamp: 1781658000000,
    time: "08:00",
    team1Id: "ARG",
    team2Id: "ALG",
    score1: 3,
    score2: 0,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Lionel Messi", min: "17'" }, { name: "Lionel Messi", min: "60'" }, { name: "Lionel Messi", min: "76'" }],
    scorers2: [],
    assists1: ["Rodrigo De Paul", "Nico González"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M20",
    group: "J",
    round: 1,
    date: "17/06/2026",
    timestamp: 1781668800000,
    time: "11:00",
    team1Id: "AUT",
    team2Id: "JOR",
    score1: 3,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Romano Schmid", min: "21'" }, { name: "Yazan Al-Arab (OG)", min: "76'" }, { name: "Marko Arnautovic", min: "90'+12'" }],
    scorers2: [{ name: "Ali Olwan", min: "50'" }],
    assists1: ["Xaver Schlager"],
    assists2: ["Noor Al-Rawabdeh"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M21",
    group: "K",
    round: 1,
    date: "18/06/2026",
    timestamp: 1781715600000,
    time: "00:00",
    team1Id: "POR",
    team2Id: "COD",
    score1: 1,
    score2: 1,
    yc1: 3,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "João Neves", min: "6'" }],
    scorers2: [{ name: "Yoane Wissa", min: "45'+5'" }],
    assists1: ["Pedro Neto"],
    assists2: ["Arthur Masuaku"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M22",
    group: "L",
    round: 1,
    date: "18/06/2026",
    timestamp: 1781726400000,
    time: "03:00",
    team1Id: "ENG",
    team2Id: "CRO",
    score1: 4,
    score2: 2,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Harry Kane", min: "12'" }, { name: "Harry Kane", min: "42'" }, { name: "Jude Bellingham", min: "47'" }, { name: "Marcus Rashford", min: "85'" }],
    scorers2: [{ name: "Martin Baturina", min: "36'" }, { name: "Petar Musa", min: "45'+5'" }],
    assists1: ["Declan Rice", "Elliot Anderson", "Bukayo Saka"],
    assists2: ["Petar Sucic", "Ivan Perisic"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M23",
    group: "L",
    round: 1,
    date: "18/06/2026",
    timestamp: 1781737200000,
    time: "06:00",
    team1Id: "GHA",
    team2Id: "PAN",
    score1: 1,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Caleb Yirenkyi", min: "90'+5'" }],
    scorers2: [],
    assists1: ["Brandon Thomas-Asante"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M24",
    group: "K",
    round: 1,
    date: "18/06/2026",
    timestamp: 1781748000000,
    time: "09:00",
    team1Id: "UZB",
    team2Id: "COL",
    score1: 1,
    score2: 3,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Abbosbek Fayzullaev", min: "60'" }],
    scorers2: [{ name: "Daniel Muñoz", min: "40'" }, { name: "Luis Díaz", min: "65'" }, { name: "Jáminton Campaz", min: "90'+9'" }],
    assists1: [],
    assists2: ["Luis Díaz", "Gustavo Puerta", "Cucho Hernández"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M25",
    group: "A",
    round: 2,
    date: "18/06/2026",
    timestamp: 1781798400000,
    time: "23:00",
    team1Id: "CZE",
    team2Id: "RSA",
    score1: 1,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Michal Sadílek", min: "6'" }],
    scorers2: [{ name: "Teboho Mokoena", min: "83'" }],
    assists1: ["Alexandr Sojka"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M26",
    group: "B",
    round: 2,
    date: "19/06/2026",
    timestamp: 1781809200000,
    time: "02:00",
    team1Id: "SUI",
    team2Id: "BIH",
    score1: 4,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 2,
    rc2: 1,
    status: "Kết thúc",
    scorers1: [{ name: "Johan Manzambi", min: "74'" }, { name: "Rubén Vargas", min: "84'" }, { name: "Johan Manzambi", min: "90'" }, { name: "Granit Xhaka", min: "90'+7'" }],
    scorers2: [{ name: "Ermin Mahmic", min: "90'+3'" }],
    assists1: ["Breel Embolo", "Rubén Vargas"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M27",
    group: "B",
    round: 2,
    date: "19/06/2026",
    timestamp: 1781820000000,
    time: "05:00",
    team1Id: "CAN",
    team2Id: "QAT",
    score1: 6,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 2,
    status: "Kết thúc",
    scorers1: [{ name: "Cyle Larin", min: "16'" }, { name: "Jonathan David", min: "29'" }, { name: "Jonathan David", min: "45'+3'" }, { name: "Nathan Saliba", min: "64'" }, { name: "Mohamed Manai (OG)", min: "75'" }, { name: "Jonathan David", min: "90'+2'" }],
    scorers2: [],
    assists1: ["Nathan Saliba"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M28",
    group: "A",
    round: 2,
    date: "19/06/2026",
    timestamp: 1781830800000,
    time: "08:00",
    team1Id: "MEX",
    team2Id: "KOR",
    score1: 1,
    score2: 0,
    yc1: 0,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Luis Romo", min: "50'" }],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M29",
    group: "D",
    round: 2,
    date: "20/06/2026",
    timestamp: 1781895600000,
    time: "02:00",
    team1Id: "USA",
    team2Id: "AUS",
    score1: 2,
    score2: 0,
    yc1: 3,
    rc1: 0,
    yc2: 4,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Cameron Burgess (OG)", min: "11'" }, { name: "Alex Freeman", min: "43'" }],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M30",
    group: "C",
    round: 2,
    date: "20/06/2026",
    timestamp: 1781906400000,
    time: "05:00",
    team1Id: "SCO",
    team2Id: "MAR",
    score1: 0,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Ismael Saibari", min: "2'" }],
    assists1: [],
    assists2: ["Brahim Díaz"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M31",
    group: "C",
    round: 2,
    date: "20/06/2026",
    timestamp: 1781915400000,
    time: "07:30",
    team1Id: "BRA",
    team2Id: "HAI",
    score1: 3,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 3,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Matheus Cunha", min: "23'" }, { name: "Matheus Cunha", min: "36'" }, { name: "Vinícius Júnior", min: "45'+3'" }],
    scorers2: [],
    assists1: ["Vinícius Júnior", "Lucas Paquetá"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M32",
    group: "D",
    round: 2,
    date: "20/06/2026",
    timestamp: 1781924400000,
    time: "10:00",
    team1Id: "TUR",
    team2Id: "PAR",
    score1: 0,
    score2: 1,
    yc1: 2,
    rc1: 0,
    yc2: 1,
    rc2: 1,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Matías Galarza", min: "2'" }],
    assists1: [],
    assists2: ["Julio Enciso"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M33",
    group: "F",
    round: 2,
    date: "21/06/2026",
    timestamp: 1781974800000,
    time: "00:00",
    team1Id: "NED",
    team2Id: "SWE",
    score1: 5,
    score2: 1,
    yc1: 0,
    rc1: 0,
    yc2: 3,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Brian Brobbey", min: "5'" }, { name: "Brian Brobbey", min: "17'" }, { name: "Cody Gakpo", min: "47'" }, { name: "Cody Gakpo", min: "54'" }, { name: "Crysencio Summerville", min: "89'" }],
    scorers2: [{ name: "Anthony Elanga", min: "59'" }],
    assists1: ["Cody Gakpo", "Denzel Dumfries", "Denzel Dumfries", "Crysencio Summerville", "Memphis Depay"],
    assists2: ["Alexander Isak"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M34",
    group: "E",
    round: 2,
    date: "21/06/2026",
    timestamp: 1781985600000,
    time: "03:00",
    team1Id: "GER",
    team2Id: "CIV",
    score1: 2,
    score2: 1,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Deniz Undav", min: "68'" }, { name: "Deniz Undav", min: "90'+4'" }],
    scorers2: [{ name: "Franck Kessié", min: "30'" }],
    assists1: ["Nadiem Amiri", "Felix Nmecha"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M35",
    group: "E",
    round: 2,
    date: "21/06/2026",
    timestamp: 1782000000000,
    time: "07:00",
    team1Id: "ECU",
    team2Id: "CUW",
    score1: 0,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 5,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M36",
    group: "F",
    round: 2,
    date: "21/06/2026",
    timestamp: 1782014400000,
    time: "11:00",
    team1Id: "TUN",
    team2Id: "JPN",
    score1: 0,
    score2: 4,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Daichi Kamada", min: "4'" }, { name: "Ayase Ueda", min: "31'" }, { name: "Junya Ito", min: "69'" }, { name: "Ayase Ueda", min: "83'" }],
    assists1: [],
    assists2: ["Keito Nakamura", "Kou Itakura", "Ayase Ueda", "Kaishu Sano"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M37",
    group: "H",
    round: 2,
    date: "21/06/2026",
    timestamp: 1782057600000,
    time: "23:00",
    team1Id: "ESP",
    team2Id: "KSA",
    score1: 4,
    score2: 0,
    yc1: 0,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Lamine Yamal", min: "10'" }, { name: "Mikel Oyarzabal", min: "21'" }, { name: "Mikel Oyarzabal", min: "24'" }, { name: "Hassan Al-Tambakti (OG)", min: "49'" }],
    scorers2: [],
    assists1: ["Mikel Oyarzabal", "Aymeric Laporte", "Dani Olmo"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M38",
    group: "G",
    round: 2,
    date: "22/06/2026",
    timestamp: 1782068400000,
    time: "02:00",
    team1Id: "BEL",
    team2Id: "IRN",
    score1: 0,
    score2: 0,
    yc1: 1,
    rc1: 1,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M39",
    group: "H",
    round: 2,
    date: "22/06/2026",
    timestamp: 1782079200000,
    time: "05:00",
    team1Id: "URU",
    team2Id: "CPV",
    score1: 2,
    score2: 2,
    yc1: 2,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Maxi Araújo", min: "44'" }, { name: "Agustín Canobbio", min: "45'+6'" }],
    scorers2: [{ name: "Kevin Pina", min: "21'" }, { name: "Hélio Varela", min: "61'" }],
    assists1: ["Maxi Araújo"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M40",
    group: "G",
    round: 2,
    date: "22/06/2026",
    timestamp: 1782090000000,
    time: "08:00",
    team1Id: "NZL",
    team2Id: "EGY",
    score1: 1,
    score2: 3,
    yc1: 2,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Finn Surman", min: "15'" }],
    scorers2: [{ name: "Mostafa Zico", min: "58'" }, { name: "Mohamed Salah", min: "67'" }, { name: "Trézéguet", min: "82'" }],
    assists1: ["Tim Payne"],
    assists2: ["Mohamed Hany", "Mostafa Zico", "Mohamed Salah"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M41",
    group: "J",
    round: 2,
    date: "23/06/2026",
    timestamp: 1782147600000,
    time: "00:00",
    team1Id: "ARG",
    team2Id: "AUT",
    score1: 2,
    score2: 0,
    yc1: 2,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Lionel Messi", min: "38'" }, { name: "Lionel Messi", min: "90'+5'" }],
    scorers2: [],
    assists1: ["Facundo Medina"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M42",
    group: "I",
    round: 2,
    date: "23/06/2026",
    timestamp: 1782162000000,
    time: "04:00",
    team1Id: "FRA",
    team2Id: "IRQ",
    score1: 3,
    score2: 0,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Kylian Mbappé", min: "14'" }, { name: "Kylian Mbappé", min: "54'" }, { name: "Ousmane Dembélé", min: "66'" }],
    scorers2: [],
    assists1: ["Michael Olise", "Ousmane Dembélé", "Michael Olise"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M43",
    group: "I",
    round: 2,
    date: "23/06/2026",
    timestamp: 1782172800000,
    time: "07:00",
    team1Id: "NOR",
    team2Id: "SEN",
    score1: 3,
    score2: 2,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Marcus Holmgren Pedersen", min: "43'" }, { name: "Erling Haaland", min: "48'" }, { name: "Erling Haaland", min: "58'" }],
    scorers2: [{ name: "Ismaïla Sarr", min: "53'" }, { name: "Ismaïla Sarr", min: "90'+3'" }],
    assists1: ["Martin Ødegaard", "Patrick Berg"],
    assists2: ["Sadio Mané", "Nicolas Jackson"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M44",
    group: "J",
    round: 2,
    date: "23/06/2026",
    timestamp: 1782183600000,
    time: "10:00",
    team1Id: "JOR",
    team2Id: "ALG",
    score1: 1,
    score2: 2,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Nizar Al-Rashdan", min: "36'" }],
    scorers2: [{ name: "Nadhir Benbouali", min: "69'" }, { name: "Amine Gouiri", min: "82'" }],
    assists1: ["Mousa Al-Tamari"],
    assists2: ["Riyad Mahrez"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M45",
    group: "K",
    round: 2,
    date: "24/06/2026",
    timestamp: 1782234000000,
    time: "00:00",
    team1Id: "POR",
    team2Id: "UZB",
    score1: 5,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Cristiano Ronaldo", min: "6'" }, { name: "Nuno Mendes", min: "17'" }, { name: "Cristiano Ronaldo", min: "39'" }, { name: "Abduvohid Nematov (OG)", min: "60'" }, { name: "Rafael Leão", min: "87'" }],
    scorers2: [],
    assists1: ["João Cancelo", "Bruno Fernandes"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M46",
    group: "L",
    round: 2,
    date: "24/06/2026",
    timestamp: 1782244800000,
    time: "03:00",
    team1Id: "ENG",
    team2Id: "GHA",
    score1: 0,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M47",
    group: "L",
    round: 2,
    date: "24/06/2026",
    timestamp: 1782255600000,
    time: "06:00",
    team1Id: "PAN",
    team2Id: "CRO",
    score1: 0,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Ante Budimir", min: "54'" }],
    assists1: [],
    assists2: ["Josip Stanisic"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M48",
    group: "K",
    round: 2,
    date: "24/06/2026",
    timestamp: 1782266400000,
    time: "09:00",
    team1Id: "COL",
    team2Id: "COD",
    score1: 1,
    score2: 0,
    yc1: 2,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Daniel Muñoz", min: "76'" }],
    scorers2: [],
    assists1: ["Juan Fernando Quintero"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M49",
    group: "B",
    round: 3,
    date: "25/06/2026",
    timestamp: 1782327600000,
    time: "02:00",
    team1Id: "BIH",
    team2Id: "QAT",
    score1: 3,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Kerim Alajbegovic", min: "29'" }, { name: "Mahmoud Abunada (OG)", min: "34'" }, { name: "Ermin Mahmic", min: "80'" }],
    scorers2: [{ name: "Hassan Al-Haydos", min: "42'" }],
    assists1: ["Ivan Basic", "Dennis Hadzikadunic"],
    assists2: ["Edmílson Junior"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M50",
    group: "B",
    round: 3,
    date: "25/06/2026",
    timestamp: 1782327600000,
    time: "02:00",
    team1Id: "SUI",
    team2Id: "CAN",
    score1: 2,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Rubén Vargas", min: "46'" }, { name: "Johan Manzambi", min: "57'" }],
    scorers2: [{ name: "Promise David", min: "76'" }],
    assists1: ["Johan Manzambi", "Breel Embolo"],
    assists2: ["Nathan Saliba"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M51",
    group: "C",
    round: 3,
    date: "25/06/2026",
    timestamp: 1782338400000,
    time: "05:00",
    team1Id: "MAR",
    team2Id: "HAI",
    score1: 4,
    score2: 2,
    yc1: 0,
    rc1: 0,
    yc2: 3,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Achraf Hakimi", min: "39'" }, { name: "Ismael Saibari", min: "45'+1'" }, { name: "Soufiane Rahimi", min: "78'" }, { name: "Gessime Yassine", min: "89'" }],
    scorers2: [{ name: "Yassine Bounou (OG)", min: "10'" }, { name: "Wilson Isidor", min: "43'" }],
    assists1: ["Achraf Hakimi", "Chadi Riad", "Soufiane Rahimi"],
    assists2: ["Jean-Kévin Duverne"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M52",
    group: "C",
    round: 3,
    date: "25/06/2026",
    timestamp: 1782338400000,
    time: "05:00",
    team1Id: "SCO",
    team2Id: "BRA",
    score1: 0,
    score2: 3,
    yc1: 1,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Vinícius Júnior", min: "7'" }, { name: "Vinícius Júnior", min: "45'+3'" }, { name: "Matheus Cunha", min: "60'" }],
    assists1: [],
    assists2: ["Rayan", "Bruno Guimarães", "Bruno Guimarães"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M53",
    group: "A",
    round: 3,
    date: "25/06/2026",
    timestamp: 1782349200000,
    time: "08:00",
    team1Id: "CZE",
    team2Id: "MEX",
    score1: 0,
    score2: 3,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Mateo Chávez", min: "55'" }, { name: "Julián Quiñones", min: "61'" }, { name: "Álvaro Fidalgo", min: "90'+4'" }],
    assists1: [],
    assists2: ["Luis Romo", "Jorge Sánchez", "Roberto Alvarado"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M54",
    group: "A",
    round: 3,
    date: "25/06/2026",
    timestamp: 1782349200000,
    time: "08:00",
    team1Id: "RSA",
    team2Id: "KOR",
    score1: 1,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Thapelo Maseko", min: "63'" }],
    scorers2: [],
    assists1: ["Tshepang Moremi"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M55",
    group: "E",
    round: 3,
    date: "26/06/2026",
    timestamp: 1782417600000,
    time: "03:00",
    team1Id: "CUW",
    team2Id: "CIV",
    score1: 0,
    score2: 2,
    yc1: 2,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Nicolas Pépé", min: "7'" }, { name: "Nicolas Pépé", min: "64'" }],
    assists1: [],
    assists2: ["Yan Diomande", "Ibrahim Sangaré"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M56",
    group: "E",
    round: 3,
    date: "26/06/2026",
    timestamp: 1782417600000,
    time: "03:00",
    team1Id: "ECU",
    team2Id: "GER",
    score1: 2,
    score2: 1,
    yc1: 3,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Nilson Angulo", min: "9'" }, { name: "Gonzalo Plata", min: "77'" }],
    scorers2: [{ name: "Leroy Sané", min: "2'" }],
    assists1: ["Pedro Vite", "Kevin Rodríguez"],
    assists2: ["Florian Wirtz"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M57",
    group: "F",
    round: 3,
    date: "26/06/2026",
    timestamp: 1782428400000,
    time: "06:00",
    team1Id: "JPN",
    team2Id: "SWE",
    score1: 1,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Daizen Maeda", min: "56'" }],
    scorers2: [{ name: "Anthony Elanga", min: "62'" }],
    assists1: ["Ritsu Doan"],
    assists2: ["Viktor Gyökeres"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M58",
    group: "F",
    round: 3,
    date: "26/06/2026",
    timestamp: 1782428400000,
    time: "06:00",
    team1Id: "TUN",
    team2Id: "NED",
    score1: 1,
    score2: 3,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Hazem Mastouri", min: "54'" }],
    scorers2: [{ name: "Ellyes Skhiri (OG)", min: "3'" }, { name: "Brian Brobbey", min: "7'" }, { name: "Jan Paul van Hecke", min: "62'" }],
    assists1: ["Hannibal Mejbri"],
    assists2: ["Virgil van Dijk", "Tijjani Reijnders"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M59",
    group: "D",
    round: 3,
    date: "26/06/2026",
    timestamp: 1782439200000,
    time: "09:00",
    team1Id: "PAR",
    team2Id: "AUS",
    score1: 0,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M60",
    group: "D",
    round: 3,
    date: "26/06/2026",
    timestamp: 1782439200000,
    time: "09:00",
    team1Id: "TUR",
    team2Id: "USA",
    score1: 3,
    score2: 2,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Arda Güler", min: "10'" }, { name: "Baris Alper Yilmaz", min: "31'" }, { name: "Kaan Ayhan", min: "90'+8'" }],
    scorers2: [{ name: "Auston Trusty", min: "3'" }, { name: "Sebastian Berhalter", min: "49'" }],
    assists1: ["Baris Alper Yilmaz", "Orkun Kökçü"],
    assists2: ["Sebastian Berhalter"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M61",
    group: "I",
    round: 3,
    date: "27/06/2026",
    timestamp: 1782500400000,
    time: "02:00",
    team1Id: "NOR",
    team2Id: "FRA",
    score1: 1,
    score2: 4,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Thelo Aasgaard", min: "21'" }],
    scorers2: [{ name: "Ousmane Dembélé", min: "7'" }, { name: "Ousmane Dembélé", min: "20'" }, { name: "Ousmane Dembélé", min: "32'" }, { name: "Désiré Doué", min: "90'+4'" }],
    assists1: ["Andreas Schjelderup"],
    assists2: ["Kylian Mbappé", "Kylian Mbappé", "Aurélien Tchouaméni", "Bradley Barcola"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M62",
    group: "I",
    round: 3,
    date: "27/06/2026",
    timestamp: 1782500400000,
    time: "02:00",
    team1Id: "SEN",
    team2Id: "IRQ",
    score1: 5,
    score2: 0,
    yc1: 2,
    rc1: 0,
    yc2: 2,
    rc2: 1,
    status: "Kết thúc",
    scorers1: [{ name: "Habib Diarra", min: "4'" }, { name: "Ismaïla Sarr", min: "56'" }, { name: "Pape Gueye", min: "59'" }, { name: "Pape Gueye", min: "71'" }, { name: "Iliman Ndiaye", min: "82'" }],
    scorers2: [],
    assists1: ["Abdoulaye Seck", "Lamine Camara", "Ismaïla Sarr", "Iliman Ndiaye", "Pape Gueye"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M63",
    group: "H",
    round: 3,
    date: "27/06/2026",
    timestamp: 1782518400000,
    time: "07:00",
    team1Id: "CPV",
    team2Id: "KSA",
    score1: 0,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 3,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M64",
    group: "H",
    round: 3,
    date: "27/06/2026",
    timestamp: 1782518400000,
    time: "07:00",
    team1Id: "URU",
    team2Id: "ESP",
    score1: 0,
    score2: 1,
    yc1: 3,
    rc1: 1,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Álex Baena", min: "42'" }],
    assists1: [],
    assists2: ["Marcos Llorente"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M65",
    group: "G",
    round: 3,
    date: "27/06/2026",
    timestamp: 1782529200000,
    time: "10:00",
    team1Id: "EGY",
    team2Id: "IRN",
    score1: 1,
    score2: 1,
    yc1: 3,
    rc1: 0,
    yc2: 4,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Mahmoud Saber", min: "5'" }],
    scorers2: [{ name: "Ramin Rezaeian", min: "14'" }],
    assists1: ["Trézéguet"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M66",
    group: "G",
    round: 3,
    date: "27/06/2026",
    timestamp: 1782529200000,
    time: "10:00",
    team1Id: "NZL",
    team2Id: "BEL",
    score1: 1,
    score2: 5,
    yc1: 2,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Elijah Just", min: "84'" }],
    scorers2: [{ name: "Leandro Trossard", min: "28'" }, { name: "Leandro Trossard", min: "50'" }, { name: "Kevin De Bruyne", min: "66'" }, { name: "Romelu Lukaku", min: "86'" }, { name: "Alexis Saelemaekers", min: "90'+4'" }],
    assists1: [],
    assists2: ["Hans Vanaken", "Nicolas Raskin", "Romelu Lukaku"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M67",
    group: "L",
    round: 3,
    date: "28/06/2026",
    timestamp: 1782594000000,
    time: "04:00",
    team1Id: "CRO",
    team2Id: "GHA",
    score1: 2,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Petar Sucic", min: "31'" }, { name: "Nikola Vlasic", min: "83'" }],
    scorers2: [{ name: "Derrick Luckassen", min: "73'" }],
    assists1: ["Mateo Kovacic", "Luka Modric"],
    assists2: ["Ernest Nuamah"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M68",
    group: "L",
    round: 3,
    date: "28/06/2026",
    timestamp: 1782594000000,
    time: "04:00",
    team1Id: "PAN",
    team2Id: "ENG",
    score1: 0,
    score2: 2,
    yc1: 2,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Jude Bellingham", min: "62'" }, { name: "Harry Kane", min: "67'" }],
    assists1: [],
    assists2: ["Bukayo Saka", "Jude Bellingham"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M69",
    group: "K",
    round: 3,
    date: "28/06/2026",
    timestamp: 1782603000000,
    time: "06:30",
    team1Id: "COL",
    team2Id: "POR",
    score1: 0,
    score2: 0,
    yc1: 1,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M70",
    group: "K",
    round: 3,
    date: "28/06/2026",
    timestamp: 1782603000000,
    time: "06:30",
    team1Id: "COD",
    team2Id: "UZB",
    score1: 3,
    score2: 1,
    yc1: 3,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Yoane Wissa", min: "68'" }, { name: "Fiston Mayele", min: "78'" }, { name: "Yoane Wissa", min: "90'+1'" }],
    scorers2: [{ name: "Eldor Shomurodov", min: "10'" }],
    assists1: ["Meschack Elia"],
    assists2: ["Akmal Mozgovoy"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M71",
    group: "J",
    round: 3,
    date: "28/06/2026",
    timestamp: 1782612000000,
    time: "09:00",
    team1Id: "ALG",
    team2Id: "AUT",
    score1: 3,
    score2: 3,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Rafik Belghali", min: "45'" }, { name: "Riyad Mahrez", min: "60'" }, { name: "Riyad Mahrez", min: "90'+3'" }],
    scorers2: [{ name: "Marko Arnautovic", min: "28'" }, { name: "Marcel Sabitzer", min: "55'" }, { name: "Sasa Kalajdzic", min: "90'+6'" }],
    assists1: ["Houssem Aouar", "Houssem Aouar"],
    assists2: ["David Alaba", "Konrad Laimer", "Michael Gregoritsch"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M72",
    group: "J",
    round: 3,
    date: "28/06/2026",
    timestamp: 1782612000000,
    time: "09:00",
    team1Id: "JOR",
    team2Id: "ARG",
    score1: 1,
    score2: 3,
    yc1: 3,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Mousa Al-Tamari", min: "55'" }],
    scorers2: [{ name: "Giovani Lo Celso", min: "19'" }, { name: "Lautaro Martínez", min: "31'" }, { name: "Lionel Messi", min: "80'" }],
    assists1: ["Ehsan Haddad"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M73",
    group: "",
    round: 32,
    date: "29/06/2026",
    timestamp: 1782673200000,
    time: "02:00",
    team1Id: "RSA",
    team2Id: "CAN",
    score1: 0,
    score2: 1,
    yc1: 0,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [],
    scorers2: [{ name: "Stephen Eustáquio", min: "90'+2'" }],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M76",
    group: "",
    round: 32,
    date: "30/06/2026",
    timestamp: 1782752400000,
    time: "00:00",
    team1Id: "BRA",
    team2Id: "JPN",
    score1: 2,
    score2: 1,
    yc1: 2,
    rc1: 0,
    yc2: 3,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Casemiro", min: "56'" }, { name: "Gabriel Martinelli", min: "90'+5'" }],
    scorers2: [{ name: "Kaishu Sano", min: "29'" }],
    assists1: ["Gabriel Magalhães", "Bruno Guimarães"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M74",
    group: "",
    round: 32,
    date: "30/06/2026",
    timestamp: 1782765000000,
    time: "03:30",
    team1Id: "GER",
    team2Id: "PAR",
    score1: 1,
    score2: 1,
    yc1: 3,
    rc1: 0,
    yc2: 3,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Kai Havertz", min: "54'" }, { name: "Joshua Kimmich", min: "120'" }, { name: "Jamal Musiala", min: "120'" }, { name: "Nadiem Amiri", min: "120'" }],
    scorers2: [{ name: "Julio Enciso", min: "42'" }, { name: "Maurício", min: "120'" }, { name: "Gustavo Gómez", min: "120'" }, { name: "Matías Galarza", min: "120'" }, { name: "José Canale", min: "120'" }],
    assists1: ["Florian Wirtz"],
    assists2: ["Matías Galarza"],
    penaltyWinner: 2,
    shootoutScore1: 3,
    shootoutScore2: 4,
    matchTime: ""
  },
  {
    id: "M75",
    group: "",
    round: 32,
    date: "30/06/2026",
    timestamp: 1782781200000,
    time: "08:00",
    team1Id: "NED",
    team2Id: "MAR",
    score1: 1,
    score2: 1,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Cody Gakpo", min: "72'" }, { name: "Teun Koopmeiners", min: "120'" }, { name: "Wout Weghorst", min: "120'" }],
    scorers2: [{ name: "Issa Diop", min: "90'+1'" }, { name: "Soufiane Rahimi", min: "120'" }, { name: "Chemsdine Talbi", min: "120'" }, { name: "Ismael Saibari", min: "120'" }],
    assists1: ["Crysencio Summerville"],
    assists2: ["Chemsdine Talbi"],
    penaltyWinner: 2,
    shootoutScore1: 2,
    shootoutScore2: 3,
    matchTime: ""
  },
  {
    id: "M78",
    group: "",
    round: 32,
    date: "01/07/2026",
    timestamp: 1782838800000,
    time: "00:00",
    team1Id: "CIV",
    team2Id: "NOR",
    score1: 1,
    score2: 2,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Amad Diallo", min: "74'" }],
    scorers2: [{ name: "Antonio Nusa", min: "39'" }, { name: "Erling Haaland", min: "86'" }],
    assists1: ["Nicolas Pépé"],
    assists2: ["Martin Ødegaard", "Patrick Berg"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M77",
    group: "",
    round: 32,
    date: "01/07/2026",
    timestamp: 1782853200000,
    time: "04:00",
    team1Id: "FRA",
    team2Id: "SWE",
    score1: 3,
    score2: 0,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Kylian Mbappé", min: "45'" }, { name: "Bradley Barcola", min: "53'" }, { name: "Kylian Mbappé", min: "74'" }],
    scorers2: [],
    assists1: ["Ousmane Dembélé", "Michael Olise", "Michael Olise"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M79",
    group: "",
    round: 32,
    date: "01/07/2026",
    timestamp: 1782871200000,
    time: "09:00",
    team1Id: "MEX",
    team2Id: "ECU",
    score1: 2,
    score2: 0,
    yc1: 0,
    rc1: 0,
    yc2: 3,
    rc2: 1,
    status: "Kết thúc",
    scorers1: [{ name: "Julián Quiñones", min: "22'" }, { name: "Raúl Jiménez", min: "31'" }],
    scorers2: [],
    assists1: ["Roberto Alvarado", "Julián Quiñones"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M80",
    group: "",
    round: 32,
    date: "01/07/2026",
    timestamp: 1782921600000,
    time: "23:00",
    team1Id: "ENG",
    team2Id: "COD",
    score1: 2,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Harry Kane", min: "75'" }, { name: "Harry Kane", min: "86'" }],
    scorers2: [{ name: "Brian Cipenga", min: "7'" }],
    assists1: ["Anthony Gordon", "Anthony Gordon"],
    assists2: ["Chancel Mbemba"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M82",
    group: "",
    round: 32,
    date: "02/07/2026",
    timestamp: 1782936000000,
    time: "03:00",
    team1Id: "BEL",
    team2Id: "SEN",
    score1: 3,
    score2: 2,
    yc1: 2,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Romelu Lukaku", min: "86'" }, { name: "Youri Tielemans", min: "89'" }, { name: "Youri Tielemans", min: "120'+5'" }],
    scorers2: [{ name: "Habib Diarra", min: "24'" }, { name: "Ismaïla Sarr", min: "51'" }],
    assists1: ["Thomas Meunier", "Leandro Trossard"],
    assists2: ["Moussa Niakhaté"],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M81",
    group: "",
    round: 32,
    date: "02/07/2026",
    timestamp: 1782950400000,
    time: "07:00",
    team1Id: "USA",
    team2Id: "BIH",
    score1: 2,
    score2: 0,
    yc1: 0,
    rc1: 1,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Folarin Balogun", min: "45'" }, { name: "Malik Tillman", min: "82'" }],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M84",
    group: "",
    round: 32,
    date: "03/07/2026",
    timestamp: 1783018800000,
    time: "02:00",
    team1Id: "ESP",
    team2Id: "AUT",
    score1: 3,
    score2: 0,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Mikel Oyarzabal", min: "36'" }, { name: "Pedro Porro", min: "66'" }, { name: "Mikel Oyarzabal", min: "89'" }],
    scorers2: [],
    assists1: ["Marc Cucurella", "Álex Baena", "Marc Cucurella"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M83",
    group: "",
    round: 32,
    date: "03/07/2026",
    timestamp: 1783033200000,
    time: "06:00",
    team1Id: "POR",
    team2Id: "CRO",
    score1: 2,
    score2: 1,
    yc1: 1,
    rc1: 0,
    yc2: 2,
    rc2: 0,
    status: "Kết thúc",
    scorers1: [{ name: "Cristiano Ronaldo", min: "68'" }, { name: "Gonçalo Ramos", min: "90'+4'" }],
    scorers2: [{ name: "Ivan Perisic", min: "53'" }],
    assists1: ["Rafael Leão"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M85",
    group: "",
    round: 32,
    date: "03/07/2026",
    timestamp: 1783047600000,
    time: "10:00",
    team1Id: "SUI",
    team2Id: "ALG",
    score1: 1,
    score2: 0,
    yc1: 0,
    rc1: 0,
    yc2: 1,
    rc2: 0,
    status: "Đang đá",
    scorers1: [{ name: "Breel Embolo", min: "10'" }],
    scorers2: [],
    assists1: ["Johan Manzambi"],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M86",
    group: "",
    round: 32,
    date: "04/07/2026",
    timestamp: 1783101600000,
    time: "01:00",
    team1Id: "AUS",
    team2Id: "EGY",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M87",
    group: "",
    round: 32,
    date: "04/07/2026",
    timestamp: 1783116000000,
    time: "05:00",
    team1Id: "ARG",
    team2Id: "CPV",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M88",
    group: "",
    round: 32,
    date: "04/07/2026",
    timestamp: 1783128600000,
    time: "08:30",
    team1Id: "COL",
    team2Id: "GHA",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M89",
    group: "",
    round: 16,
    date: "05/07/2026",
    timestamp: 1783184400000,
    time: "00:00",
    team1Id: "CAN",
    team2Id: "MAR",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M90",
    group: "",
    round: 16,
    date: "05/07/2026",
    timestamp: 1783198800000,
    time: "04:00",
    team1Id: "PAR",
    team2Id: "FRA",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M91",
    group: "",
    round: 16,
    date: "06/07/2026",
    timestamp: 1783281600000,
    time: "03:00",
    team1Id: "BRA",
    team2Id: "NOR",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M92",
    group: "",
    round: 16,
    date: "06/07/2026",
    timestamp: 1783296000000,
    time: "07:00",
    team1Id: "MEX",
    team2Id: "ENG",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M93",
    group: "",
    round: 16,
    date: "07/07/2026",
    timestamp: 1783364400000,
    time: "02:00",
    team1Id: "POR",
    team2Id: "ESP",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M94",
    group: "",
    round: 16,
    date: "07/07/2026",
    timestamp: 1783382400000,
    time: "07:00",
    team1Id: "USA",
    team2Id: "BEL",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M95",
    group: "",
    round: 16,
    date: "07/07/2026",
    timestamp: 1783440000000,
    time: "23:00",
    team1Id: "RD32",
    team2Id: "RD32",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M96",
    group: "",
    round: 16,
    date: "08/07/2026",
    timestamp: 1783454400000,
    time: "03:00",
    team1Id: "RD32",
    team2Id: "RD32",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M97",
    group: "",
    round: 8,
    date: "10/07/2026",
    timestamp: 1783627200000,
    time: "03:00",
    team1Id: "RD16 W1",
    team2Id: "RD16 W2",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M99",
    group: "",
    round: 8,
    date: "11/07/2026",
    timestamp: 1783710000000,
    time: "02:00",
    team1Id: "RD16 W5",
    team2Id: "RD16 W6",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M98",
    group: "",
    round: 8,
    date: "12/07/2026",
    timestamp: 1783803600000,
    time: "04:00",
    team1Id: "RD16 W3",
    team2Id: "RD16 W4",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M100",
    group: "",
    round: 8,
    date: "12/07/2026",
    timestamp: 1783818000000,
    time: "08:00",
    team1Id: "RD16 W7",
    team2Id: "RD16 W8",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M101",
    group: "",
    round: 4,
    date: "15/07/2026",
    timestamp: 1784055600000,
    time: "02:00",
    team1Id: "QFW1",
    team2Id: "QFW2",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M102",
    group: "",
    round: 4,
    date: "16/07/2026",
    timestamp: 1784142000000,
    time: "02:00",
    team1Id: "QFW3",
    team2Id: "QW4",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M103",
    group: "",
    round: 2,
    date: "19/07/2026",
    timestamp: 1784408400000,
    time: "04:00",
    team1Id: "SF L1",
    team2Id: "SF L2",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
  {
    id: "M104",
    group: "",
    round: 1,
    date: "20/07/2026",
    timestamp: 1784487600000,
    time: "02:00",
    team1Id: "SFW1",
    team2Id: "SFW2",
    score1: null,
    score2: null,
    yc1: 0,
    rc1: 0,
    yc2: 0,
    rc2: 0,
    status: "Chưa đấu",
    scorers1: [],
    scorers2: [],
    assists1: [],
    assists2: [],
    penaltyWinner: null,
    shootoutScore1: null,
    shootoutScore2: null,
    matchTime: ""
  },
];

// Hàm bổ trợ tìm kiếm thông tin chi tiết của đội bóng theo ID
function findTeamById(teamId) {
  for (const groupLetter of Object.keys(WORLD_CUP_DATA.groups)) {
    const team = WORLD_CUP_DATA.groups[groupLetter].find(t => t.id === teamId);
    if (team) return team;
  }
  return null;
}

// Khởi tạo DEFAULT_MATCHES với đầy đủ thông tin tên tiếng Việt, cờ và cờ hiệu
const DEFAULT_MATCHES = OFFICIAL_MATCHES_RAW.map(match => {
  const t1 = findTeamById(match.team1Id);
  const t2 = findTeamById(match.team2Id);
  return {
    ...match,
    team1: t1 ? t1.name : "",
    team1FlagCode: t1 ? t1.flagCode : "",
    team1Flag: t1 ? t1.flag : "",
    team2: t2 ? t2.name : "",
    team2FlagCode: t2 ? t2.flagCode : "",
    team2Flag: t2 ? t2.flag : "",
    matchTime: match.matchTime || ""
  };
});

// Dữ liệu thống kê cầu thủ World Cup 2026 (Mặc định khởi tạo 0, tự động tích lũy từ kết quả đấu)
const PLAYER_STATS_DATA = [
  { name: "Kylian Mbappé", team: "Pháp", flagCode: "fr", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 },
  { name: "Harry Kane", team: "Anh", flagCode: "gb-eng", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 },
  { name: "Vinícius Júnior", team: "Brazil", flagCode: "br", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 },
  { name: "Jude Bellingham", team: "Anh", flagCode: "gb-eng", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 },
  { name: "Erling Haaland", team: "Na Uy", flagCode: "no", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 },
  { name: "Son Heung-min", team: "Hàn Quốc", flagCode: "kr", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 },
  { name: "Jamal Musiala", team: "Đức", flagCode: "de", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 },
  { name: "Kevin De Bruyne", team: "Bỉ", flagCode: "be", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 },
  { name: "Bruno Fernandes", team: "Bồ Đào Nha", flagCode: "pt", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 },
  { name: "Lionel Messi", team: "Argentina", flagCode: "ar", goals: 0, assists: 0, xg: 0.0, keyPasses: 0 }
];

// Danh sách các cầu thủ tiêu biểu của 48 quốc gia tham dự World Cup 2026
const TEAM_PLAYERS = {
  "Mexico": [
    { name: "Santiago Giménez", flagCode: "mx" },
    { name: "Hirving Lozano", flagCode: "mx" },
    { name: "Edson Álvarez", flagCode: "mx" }
  ],
  "Hàn Quốc": [
    { name: "Son Heung-min", flagCode: "kr" },
    { name: "Hwang Hee-chan", flagCode: "kr" },
    { name: "Lee Kang-in", flagCode: "kr" }
  ],
  "Nam Phi": [
    { name: "Percy Tau", flagCode: "za" },
    { name: "Themba Zwane", flagCode: "za" }
  ],
  "Cộng hòa Séc": [
    { name: "Patrik Schick", flagCode: "cz" },
    { name: "Tomáš Souček", flagCode: "cz" }
  ],
  "Canada": [
    { name: "Jonathan David", flagCode: "ca" },
    { name: "Alphonso Davies", flagCode: "ca" },
    { name: "Cyle Larin", flagCode: "ca" }
  ],
  "Thụy Sĩ": [
    { name: "Breel Embolo", flagCode: "ch" },
    { name: "Xherdan Shaqiri", flagCode: "ch" },
    { name: "Granit Xhaka", flagCode: "ch" }
  ],
  "Qatar": [
    { name: "Akram Afif", flagCode: "qa" },
    { name: "Almoez Ali", flagCode: "qa" }
  ],
  "Bosnia-Herzegovina": [
    { name: "Edin Džeko", flagCode: "ba" },
    { name: "Miralem Pjanić", flagCode: "ba" }
  ],
  "Brazil": [
    { name: "Vinícius Júnior", flagCode: "br" },
    { name: "Rodrygo", flagCode: "br" },
    { name: "Neymar Jr", flagCode: "br" }
  ],
  "Morocco": [
    { name: "Youssef En-Nesyri", flagCode: "ma" },
    { name: "Hakim Ziyech", flagCode: "ma" },
    { name: "Achraf Hakimi", flagCode: "ma" }
  ],
  "Scotland": [
    { name: "Scott McTominay", flagCode: "gb-sct" },
    { name: "John McGinn", flagCode: "gb-sct" }
  ],
  "Haiti": [
    { name: "Frantzdy Pierrot", flagCode: "ht" },
    { name: "Duckens Nazon", flagCode: "ht" }
  ],
  "Mỹ": [
    { name: "Christian Pulisic", flagCode: "us" },
    { name: "Folarin Balogun", flagCode: "us" },
    { name: "Timothy Weah", flagCode: "us" }
  ],
  "Paraguay": [
    { name: "Miguel Almirón", flagCode: "py" },
    { name: "Julio Enciso", flagCode: "py" }
  ],
  "Úc": [
    { name: "Craig Goodwin", flagCode: "au" },
    { name: "Mitchell Duke", flagCode: "au" }
  ],
  "Thổ Nhĩ Kỳ": [
    { name: "Arda Güler", flagCode: "tr" },
    { name: "Hakan Çalhanoğlu", flagCode: "tr" },
    { name: "Cenk Tosun", flagCode: "tr" }
  ],
  "Đức": [
    { name: "Jamal Musiala", flagCode: "de" },
    { name: "Florian Wirtz", flagCode: "de" },
    { name: "Niclas Füllkrug", flagCode: "de" }
  ],
  "Ecuador": [
    { name: "Enner Valencia", flagCode: "ec" },
    { name: "Kendry Páez", flagCode: "ec" }
  ],
  "Bờ Biển Ngà": [
    { name: "Sébastien Haller", flagCode: "ci" },
    { name: "Simon Adingra", flagCode: "ci" }
  ],
  "Curacao": [
    { name: "Juninho Bacuna", flagCode: "cw" },
    { name: "Rangelo Janga", flagCode: "cw" }
  ],
  "Hà Lan": [
    { name: "Cody Gakpo", flagCode: "nl" },
    { name: "Memphis Depay", flagCode: "nl" },
    { name: "Xavi Simons", flagCode: "nl" }
  ],
  "Nhật Bản": [
    { name: "Kaoru Mitoma", flagCode: "jp" },
    { name: "Takefusa Kubo", flagCode: "jp" },
    { name: "Ayase Ueda", flagCode: "jp" }
  ],
  "Tunisia": [
    { name: "Youssef Msakni", flagCode: "tn" },
    { name: "Montassar Talbi", flagCode: "tn" }
  ],
  "Thụy Điển": [
    { name: "Alexander Isak", flagCode: "se" },
    { name: "Viktor Gyökeres", flagCode: "se" },
    { name: "Dejan Kulusevski", flagCode: "se" }
  ],
  "Bỉ": [
    { name: "Kevin De Bruyne", flagCode: "be" },
    { name: "Romelu Lukaku", flagCode: "be" },
    { name: "Leandro Trossard", flagCode: "be" }
  ],
  "Iran": [
    { name: "Mehdi Taremi", flagCode: "ir" },
    { name: "Sardar Azmoun", flagCode: "ir" }
  ],
  "Ai Cập": [
    { name: "Mohamed Salah", flagCode: "eg" },
    { name: "Mostafa Mohamed", flagCode: "eg" }
  ],
  "New Zealand": [
    { name: "Chris Wood", flagCode: "nz" },
    { name: "Sarpreet Singh", flagCode: "nz" }
  ],
  "Tây Ban Nha": [
    { name: "Lamine Yamal", flagCode: "es" },
    { name: "Álvaro Morata", flagCode: "es" },
    { name: "Nico Williams", flagCode: "es" }
  ],
  "Uruguay": [
    { name: "Darwin Núñez", flagCode: "uy" },
    { name: "Federico Valverde", flagCode: "uy" },
    { name: "Luis Suárez", flagCode: "uy" }
  ],
  "Saudi Arabia": [
    { name: "Salem Al-Dawsari", flagCode: "sa" },
    { name: "Firas Al-Buraikan", flagCode: "sa" }
  ],
  "Cape Verde": [
    { name: "Ryan Mendes", flagCode: "cv" },
    { name: "Bebé", flagCode: "cv" }
  ],
  "Pháp": [
    { name: "Kylian Mbappé", flagCode: "fr" },
    { name: "Antoine Griezmann", flagCode: "fr" },
    { name: "Olivier Giroud", flagCode: "fr" }
  ],
  "Senegal": [
    { name: "Sadio Mané", flagCode: "sn" },
    { name: "Nicolas Jackson", flagCode: "sn" }
  ],
  "Na Uy": [
    { name: "Erling Haaland", flagCode: "no" },
    { name: "Martin Ødegaard", flagCode: "no" }
  ],
  "Iraq": [
    { name: "Aymen Hussein", flagCode: "iq" },
    { name: "Ali Jasim", flagCode: "iq" }
  ],
  "Argentina": [
    { name: "Lionel Messi", flagCode: "ar" },
    { name: "Lautaro Martínez", flagCode: "ar" },
    { name: "Julián Álvarez", flagCode: "ar" }
  ],
  "Áo": [
    { name: "Michael Gregoritsch", flagCode: "at" },
    { name: "Marcel Sabitzer", flagCode: "at" }
  ],
  "Algeria": [
    { name: "Riyad Mahrez", flagCode: "dz" },
    { name: "Baghdad Bounedjah", flagCode: "dz" }
  ],
  "Jordan": [
    { name: "Musa Al-Taamari", flagCode: "jo" },
    { name: "Yazan Al-Naimat", flagCode: "jo" }
  ],
  "Bồ Đào Nha": [
    { name: "Cristiano Ronaldo", flagCode: "pt" },
    { name: "Bruno Fernandes", flagCode: "pt" },
    { name: "Rafael Leão", flagCode: "pt" }
  ],
  "Colombia": [
    { name: "Luis Díaz", flagCode: "co" },
    { name: "James Rodríguez", flagCode: "co" }
  ],
  "Uzbekistan": [
    { name: "Eldor Shomurodov", flagCode: "uz" },
    { name: "Otabek Shukurov", flagCode: "uz" }
  ],
  "CH Dân Chủ Congo": [
    { name: "Yoane Wissa", flagCode: "cd" },
    { name: "Cédric Bakambu", flagCode: "cd" }
  ],
  "Anh": [
    { name: "Harry Kane", flagCode: "gb-eng" },
    { name: "Jude Bellingham", flagCode: "gb-eng" },
    { name: "Bukayo Saka", flagCode: "gb-eng" }
  ],
  "Croatia": [
    { name: "Andrej Kramarić", flagCode: "hr" },
    { name: "Luka Modrić", flagCode: "hr" }
  ],
  "Panama": [
    { name: "Cecilio Waterman", flagCode: "pa" },
    { name: "Ismael Díaz", flagCode: "pa" }
  ],
  "Ghana": [
    { name: "Inaki Williams", flagCode: "gh" },
    { name: "Mohammed Kudus", flagCode: "gh" }
  ]
};